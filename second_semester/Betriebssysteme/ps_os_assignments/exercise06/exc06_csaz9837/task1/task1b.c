...
void *thread_func(void *arg) {
    //reinitialize the struct in order to acces the same values
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        //joins with terminated thread, if pthread_join is succesful we can be sure that the thread has been terminated
