...
function explicitStep(us::Vector, h::Float64, tau::Float64)::Vector
    ...
    sizeOfGrid = length(us)
    #new array with zero for the new values
    usNew = zeros(Float64, sizeOfGrid)
    usNew[1] = us[1]
    usNew[end] = us[end]
    for i in 2:(sizeOfGrid-1) #mashallah
        usNew[i] = (((us[i+1]-2*us[i]+us[i-1])*tau)/h^2) + us[i] 
    end
    return usNew;
end
