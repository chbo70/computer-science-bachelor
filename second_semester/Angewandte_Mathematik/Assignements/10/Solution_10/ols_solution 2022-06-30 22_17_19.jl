...
function linear_approximation_error(x,y,c_linear)
    ...
    for (i,t) in enumerate(x)
        pol_val = c_linear[2] * t + c_linear[1];
        error += (y[i] - pol_val) * (y[i] - pol_val);
    end
    return error;
end
...
function quadratic_approximation_error(x,y,c_quadratic)
    error = 0;
    for (i,t) in enumerate(x)
        pol_val = c_quadratic[3] * t * t + c_quadratic[2] * t + c_quadratic[1];
        error += (y[i] - pol_val) * (y[i] - pol_val);
    end
    return error;
end
...
function cubic_approximation_error(x,y,c_cubic)
    error = 0;
    for (i,t) in enumerate(x)
        pol_val = c_cubic[4] * t * t * t + c_cubic[3] * t * t + c_cubic[2] * t + c_cubic[1];
        error += (y[i] - pol_val) * (y[i] - pol_val);
    end
    return error;
end

# compare & plot linear, quadratic & cubic fits w.r.t. data points
...
function quadratic_ls_prediction(x,y,c_quadratic)
    ...
    for (i,t) in enumerate(x)
        ...
        error += (y[i] - predictions[i]) * (y[i] - predictions[i]);
    end
